 ...
 *  `v` ("vote-seen") is the Home feed spec's "don't repeatedly show the same
 *  Vote during one browsing session" (§4) — event ids whose Vote card has
 *  already been served THIS session, across every tab that shows one. */
interface Cursor { u?: string; e?: number; s?: string[]; v?: string[]; }
